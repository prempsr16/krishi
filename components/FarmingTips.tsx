...
const FarmingTips: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-4 text-gray-600">Loading farming tips...</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <h2 className="text-3xl font-bold text-gray-800 mb-6">Seasonal Farming Tips</h2>
          ...
          <div key={tip.id} className="bg-white p-6 rounded-xl shadow-md">
            <div className="flex justify-between items-center">
                ...
                <button 
                    ...
                    disabled={audioLoadingId === tip.id}
                    className="p-2 rounded-full hover:bg-green-100 transition disabled:opacity-50"
                >
                    {audioLoadingId === tip.id ? <Spinner className="w-5 h-5 text-green-600" /> : <VolumeUpIcon className="w-5 h-5 text-green-600" />}
                </button>
            </div>
