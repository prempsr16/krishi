...
const Home: React.FC<HomeProps> = ({ setActiveTab }) => {
  ...
  const fetchDailyTip = useCallback(async () => {
    setLoadingTip(true);
    const tip = await generateText("Provide a single, short, and encouraging farming tip of the day for a farmer in India.", language);
    setDailyTip(tip);
    setLoadingTip(false);
  }, [language]);
  ...
  return (
    <div className="p-4 md:p-8 space-y-8">
      <div className="bg-gradient-to-r from-green-500 to-lime-500 text-white p-6 rounded-xl shadow-lg">
        <h2 className="text-3xl font-bold">{t('Welcome back, Farmer!')}</h2>
        <p className="mt-2 text-lg">{t('Your trusted agricultural assistant is here to help you grow more.')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <button onClick={() => setActiveTab(Tab.CropDiagnosis)} className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-800">{t('Upload Crop Photo')}</h3>
          <p className="text-gray-500 mt-1">{t('Get instant diagnosis for diseases.')}</p>
        </button>
        <button onClick={() => setActiveTab(Tab.Chat)} className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-center">
          <div className="text-4xl mb-3">💬</div>
          <h3 className="text-xl font-semibold text-gray-800">{t('Ask a Question')}</h3>
          <p className="text-gray-500 mt-1">{t('Get answers to your farming queries.')}</p>
        </button>
        <button onClick={() => setActiveTab(Tab.MarketInsights)} className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-center">
          <div className="text-4xl mb-3">📈</div>
          <h3 className="text-xl font-semibold text-gray-800">{t('View Market Rates')}</h3>
          <p className="text-gray-500 mt-1">{t('Check the latest prices for your crops.')}</p>
        </button>
      </div>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl font-semibold text-gray-800 border-b pb-3 mb-3">💡 {t('Daily Tip')}</h3>
            {loadingTip ? (
              <div className="flex items-center space-x-2 text-gray-500">
                <Spinner className="w-5 h-5" />
                <span>{t('Generating your daily tip...')}</span>
              </div>
            ) : (
        ...
        <div className="bg-white p-6 rounded-xl shadow-md">
            <h3 className="text-xl font-semibold text-gray-800 border-b pb-3 mb-3">🌦️ {t('Weather Summary')}</h3>
